Capybara::SpecHelper.spec Capybara::Window, requires: [:windows] do
  ...
  describe '#maximize' do
    it 'should be able to maximize window', requires: [:js] do
      width, height = @session.evaluate_script("[window.screen.availWidth, window.screen.availHeight];")
      window = @session.current_window
      window.resize_to(width-100, height-100)
      expect(@session.evaluate_script("[window.outerWidth, window.outerHeight];")).to eq([width-100, height-100])
      window.maximize
      sleep 0.6 # Selenium returns when window isn't yet fully resized
      expect(@session.evaluate_script("[window.outerWidth, window.outerHeight];")).to eq([width, height])
    end
